...
##### by implementing the two optimizations described above (lazy updates and union-by-rank), the total cost is bounded above by O(mlg∗ n), where lg∗ n is the number of times you need to take log<sub>2</sub> until you get down to 1.
We can now implement the operations as follows:
...
### owing to above properties comes the following theorem-> Using link-by-rank, any UNION or FIND operations takes O(log n) time in the worst case, where n is the number of elements. The running time of each operation is bounded by the tree height. By the PROPERTY 5, the height is ≤ lower_bound(log<sub>2</sub>n).
